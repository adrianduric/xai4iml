...
def test_ensemble(dataset_name, augmented_data, num_runs, explanation_type=None, peer_model_name=None):
    ...
    for run in range(num_runs):
        ...
        # Collecting predictions and targets of all models
        all_preds_per_model = []
        all_targets_per_model = []

        # Collecting loss for average loss calculation
        total_loss_all_models = 0

        # Initializing each model, and performing inference with them
        for model_name in model_names:
            ...
            all_preds_per_model.append(all_predictions)
            all_targets_per_model.append(all_targets)
            total_loss_all_models += total_loss

        # Asserting that all images were given in the correct order
        y = all_targets_per_model[0]
        for target in all_targets_per_model:
            assert torch.equal(target, y)

        # Stacking model predictions in one tensor
        all_preds_per_model = torch.stack(all_preds_per_model, dim=0) # [num_models, num_samples, num_classes], e.g. [8, n, 23]

        # Average the logits
        averaged_preds = torch.mean(all_preds_per_model, dim=0) # [num_samples, num_classes], e.g. [n, 23]

        # Using averaged predictions and targets to calculate metrics
        update_metrics(preds=averaged_preds, y=y)

        ensemble_metrics = compute_metrics(
            total_loss=total_loss_all_models,
            num_samples=len(y) * len(model_names)
        )

        all_run_metrics.append(ensemble_metrics)
